...
class TpDenyCommand extends BaseCommand {
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void {
        ...
        if (isset($args["player"])) {
            ...
            $this->deny($request, $sender, $session);
            return;
        }
    ...
    public function deny(TeleportRequest $request, Player $sender, Session $session): void {
        $event = new TeleportRespondEvent($sender, false);
        $event->call();

        if ($event->isCancelled()) return;

        $request->getSender()->sendMessage(TextFormat::colorize("&6Teleport request denied."));
